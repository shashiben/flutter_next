        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            NextGridView(
              ...
              children:
                  List.generate(10, (index) => _exampleGridItem(index + 1)),
            ),
            ...
            NextGridView(
              ...
              children: List.generate(
                  8,
                  (index) =>
                      _exampleGridItem(index + 1, color: Colors.teal.shade100)),
            ),
            const SizedBox(height: 20),
            _buildSectionTitle(context,
                'GridView with Fixed Item Count per Row (Non-Responsive)'),
            _buildExplanation(
                context,
            ...
            _buildExplanation(
                context, 'Example: Always 2 items per row (using 50% for xs)'),
            NextGridView(
              mainAxisExtent: 70,
              widthPercentages: const {
              ...
              children: List.generate(
                  6,
                  (index) => _exampleGridItem(index + 1,
                      color: Colors.orange.shade100)),
            ),
            const SizedBox(height: 10),
            _buildExplanation(context,
                'Example: Always 3 items per row (using 33.33% for xs)'),
            NextGridView(
              mainAxisExtent: 70,
              mainAxisSpacing: 5,
              crossAxisSpacing: 5,
            ...
            const SizedBox(height: 20),
            _buildSectionTitle(
                context, 'GridView with Many Items (Scroll Performance)'),
            _buildExplanation(
                context,
            ...
            SizedBox(
              // Constrain height to demonstrate internal scrollability if needed, though here it will just make the page longer.
